...
- **If `docker compose up` fails with `error mounting .../docker/nginx/default.conf ... not a directory`**, Docker auto-created an empty directory where the file should be (this happens when the file doesnt exist locally, e.g. after pulling a commit that removed it from git tracking). Fix: `sudo rm -rf docker/nginx/default.conf` (needs sudo since Docker creates it as root), then `cp docker/nginx/default.conf.example docker/nginx/default.conf` and confirm it uses `fastcgi_pass app:9000;` for local dev.
- **If `docker compose up` fails with `error mounting .../docker/nginx/default.conf ... not a directory`**, Docker auto-created an empty directory where the file should be (this happens when the file doesnt exist locally, e.g. after pulling a commit that removed it from git tracking). Fix: `sudo rm -rf docker/nginx/default.conf` (needs sudo since Docker creates it as root), then `cp docker/nginx/default.conf.example docker/nginx/default.conf` and confirm it uses `fastcgi_pass app:9000;` for local dev.
